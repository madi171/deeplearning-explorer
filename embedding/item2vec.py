...
input_file = "datasets/video_watch_seq.out"
output = "output/video_watch_seq.result"
...
def get_title(vid):
    return urllib2.urlopen(url % vid).read()
...
for i in xrange(1, 50):
    ...
    if lens > 350 or lens < 14:
        continue
    print "# %d target video = %s, %s" % (i, target_title, target_vid)
    ...
    for ss in sim_list:
        vid = ss[0]
        title = get_title(vid)
        print "  %d sim %.2f video = %s, %s" % (i, ss[1], title, vid)
    print "\n\n"
